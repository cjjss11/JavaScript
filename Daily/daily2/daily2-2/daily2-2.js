...
// 1. forEach 메서드를 활용하여 이름 출력
users.forEach(ele=>console.log(ele.name));

// 2. filter 메서드를 활용해 결혼한 사람들만 모아 married 상수에 할당
const married = users.filter((user) => user.isMarried === true)
console.log(married)

// 3. find 메서드를 활용해 이름이 Tom인 사람만 tom 상수에 할당
const tom = users.find((user) => user.name === 'Tom')
console.log(tom)

// 4. map 메서드를 활용해 모든 사용자에게 isAlive 키를 추가하고 true로 설정한 뒤, newUsers 상수에 할당
let newUsers = []
...
// 5. reduce 메서드를 활용해 모든 사용자들의 계좌잔액을 totalBalance 상수에 할당
